...
void TaskB(void *argument)
{
	while(1)
	{
		//	SEGGER_SYSVIEW_Printf("Attemp_take_semaphore");
		if(xSemaphoreTake(semPtr, 500 / portTICK_PERIOD_MS) == pdPASS)
		{
			RedLed_Off();
		//	SEGGER_SYSVIEW_Printf("Received_Semaphore");
			BlueTripleBlink();
		}
		else
		{
			RedLed_On();
		//	SEGGER_SYSVIEW_Printf("Failed_to_Received_Semaphore");
		}
	}
}
