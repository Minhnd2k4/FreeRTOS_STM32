...
typedef struct {
	...
	uint8_t blueREDState : 1;
	uint32_t msDelaytime;
	char Message[MAX_SIZE_LENGTH];
}LedStates_t;
...
/* USER CODE BEGIN PV */
LedStates_t LedState1 = {1, 0, 1, 200, "Hello from LedState1"};
LedStates_t LedState2 = {0, 1, 0, 300, "Hello from LedState2"};
/* USER CODE END PV */
...
int main(void)
{
  ...
  assert_param(ledCmdQueue);

  if(xTaskCreate(recvTask, "recvTask", STACK_SIZE, NULL, tskIDLE_PRIORITY + 2, NULL) != pdPASS)
  {
...
void recvTask(void *NotUsed)
{
	...
	while(1)
	{
		if(xQueueReceive(ledCmdQueue, &nextCmd, portMAX_DELAY) == pdPASS)
		{
...
void sendingTask(void *Notused)
{
	LedStates_t* nextState1 = &LedState1;
	LedStates_t* nextState2 = &LedState2;
	while(1)
	{
		xQueueSend(ledCmdQueue, &nextState1, portMAX_DELAY);
		xQueueSend(ledCmdQueue, &nextState2, portMAX_DELAY);
	}
}
/* USER CODE END 4 */
