...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void UartTaskPoll(void *argument);
void TaskPrintf(void *argument);

QueueHandle_t CmdUartQueue = NULL;
/* USER CODE END 0 */
...
void UartTaskPoll(void *argument)
{
	while (1)
	{
		if (HAL_UART_Receive(&huart1, &rx_byte, sizeof(rx_byte), 10) == HAL_OK)
		{
			rx_data[rx_index++] = rx_byte;

			if (rx_index >= FRAME_SIZE)  // Đủ 8 byte rồi
			{
				if (rx_data[FRAME_SIZE - 1] == 0xAA)  // Kiểm tra byte kết thúc
				{
					xQueueSend(CmdUartQueue, rx_data, portMAX_DELAY);
				}
				rx_index = 0;  // Reset sau khi gửi
			}
		}
...
void assert_failed(uint8_t *file, uint32_t line)
{
  ...
  /* USER CODE END 6 */
	HAL_UART_Transmit(&huart1, (uint8_t*)"Create_Error", strlen("Create_Error"), 10);
}
